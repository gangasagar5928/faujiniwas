...
def clean_content(raw_text):
    ...
    for line in lines:
        ...
        if capture:
            # Match line number format: "1: // Fauji Niwas..."
            match = re.match(r'^\d+:\s?(.*)$', line)
            if match:
                cleaned.append(match.group(1))
            else:
                # If it doesn't match the format for some reason, just append it
                if line.strip():
                    cleaned.append(line)
    return '\n'.join(cleaned)
    ...
    for line in f:
        try:
            ...
            if step.get('source') == 'SYSTEM' and step.get('type') == 'TOOL_RESPONSE':
                ...
                if 'securityEngine.js' in content and 'File Path:' in content:
                    recovered_content['securityEngine.js'] = clean_content(content)
                elif 'AdminPanel.jsx' in content and 'File Path:' in content:
