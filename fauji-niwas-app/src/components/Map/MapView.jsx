...
import { SafeMarker, SafeCircle } from './SafeMarker';
...
function AutoPoiLayers({ activeCity, showCommuteZones, showHospitals, showSchools, showCanteens, activeCantt }) {
  ...
  return (
    <>
      ...
      {showPoi && (
        <>
          {showHospitals && MILITARY_HOSPITALS.filter(h => h.lat && h.lng && !Number.isNaN(parseFloat(h.lat)) && !Number.isNaN(parseFloat(h.lng))).map((h, i) => (
            <SafeMarker key={`mh-${i}`} position={[h.lat, h.lng]} icon={MH_ICON}>
              <Popup><div className={styles.popup}>🏥 {h.name}</div></Popup>
            </SafeMarker>
          ))}

          {showSchools && ARMY_SCHOOLS.filter(s => s.lat && s.lng && !Number.isNaN(parseFloat(s.lat)) && !Number.isNaN(parseFloat(s.lng))).map((s, i) => (
            <SafeMarker key={`sch-${i}`} position={[s.lat, s.lng]} icon={SCHOOL_ICON}>
              <Popup><div className={styles.popup}>🏫 {s.name}</div></Popup>
            </SafeMarker>
          ))}

          {showCanteens && CANTEENS.filter(c => c.lat && c.lng && !Number.isNaN(parseFloat(c.lat)) && !Number.isNaN(parseFloat(c.lng))).map((c, i) => (
            <SafeMarker key={`canteen-${i}`} position={[c.lat, c.lng]} icon={CANTEEN_ICON}>
