...
export default function DetailModal({ id, onClose }) {
  ...
  const rawDaysAgo = r.createdAt ? Math.floor((Date.now() - r.createdAt) / 86400000) : null;
  const daysAgo = (rawDaysAgo !== null && !Number.isNaN(rawDaysAgo) && isFinite(rawDaysAgo)) ? rawDaysAgo : null;
  ...
  const arbResult = Number.isNaN(arbRatio) || !isFinite(arbRatio) ? { label: '⚖️ Fair Value', color: '#f4c542', text: 'Pricing details unavailable.' } :
  ...
  const safeArbPct = !Number.isNaN(arbRatio) && isFinite(arbRatio) ? Math.min(100, arbRatio * 100) : 0;
  ...
  return (
    <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && onClose()}>
      ...
      <div className="mc glass-tactical" style={{ display: 'flex', flexDirection: 'column', height: '88vh' }}>
          ...
          <div className={styles.body}>
            ...
            <div className={`${styles.arbCard} bg-mesh`}>
              ...
              <div style={{display:'flex', justifyContent:'space-between', fontSize:10, color:'var(--muted)', marginBottom:8}}>
                <span>₹0</span>
                <span style={{color: arbResult.color, fontWeight:700}}>{Math.round(safeArbPct)}% of ₹{(Number(userHra) || 0).toLocaleString()} HRA</span>
                <span>100%+</span>
              </div>
        ...
        {showPremiumModal && (
          <div className="modal-backdrop" style={{ zIndex: 99999, background: 'rgba(0,0,0,0.85)' }} onClick={() => setShowPremiumModal(false)}>
            <div className="mc glass-tactical" style={{ maxWidth: 440, width: '90%', padding: 24, borderRadius: 18, border: '1px solid var(--gold)', background: 'rgba(10,15,30,0.98)', boxShadow: '0 0 25px rgba(255,215,0,0.2)', textAlign: 'center' }} onClick={e => e.stopPropagation()}>
              <div style={{ fontSize: 32, marginBottom: 12 }}>🎖️</div>
              <h3 style={{ fontSize: 18, fontWeight: 800, color: 'var(--gold)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>Fauji Premium Suite</h3>
              <p style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 18 }}>Exclusively designed for defence personnel & verified civilian hosts.</p>
              
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, textAlign: 'left', marginBottom: 20 }}>
                <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  <span style={{ fontSize: 14 }}>⚡</span>
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 'bold', color: '#fff' }}>24-Hour Early Access</div>
                    <div style={{ fontSize: 10, color: 'var(--muted)' }}>View high-demand cantonment accommodations before they go public.</div>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  <span style={{ fontSize: 14 }}>📑</span>
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 'bold', color: '#fff' }}>Verified Title Deeds</div>
                    <div style={{ fontSize: 10, color: 'var(--muted)' }}>Instantly view audited registry proof logs from official land registries.</div>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  <span style={{ fontSize: 14 }}>⚖️</span>
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 'bold', color: '#fff' }}>Legal Break-Clause Lease Drafts</div>
                    <div style={{ fontSize: 10, color: 'var(--muted)' }}>Standard templates pre-loaded with the Indian military sudden-vacate clause.</div>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  <span style={{ fontSize: 14 }}>📞</span>
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 'bold', color: '#fff' }}>Military Helpline Support</div>
                    <div style={{ fontSize: 10, color: 'var(--muted)' }}>Get direct support for outliving allowance verification or disputes.</div>
                  </div>
                </div>
              </div>
              
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <button className="bp" style={{ width: '100%', background: 'linear-gradient(135deg, var(--gold), #f59e0b)', color: '#000', fontWeight: 'bold', padding: 12 }} onClick={() => { setShowPremiumModal(false); ctx.showToast('Premium upgrade processing! 🎖️', 'ok'); }}>
                  Upgrade for ₹199 / Month
                </button>
                <button className="fb" style={{ width: '100%', padding: 10, color: 'var(--muted)', fontSize: 11, border: '1px solid var(--border2)' }} onClick={() => { setShowPremiumModal(false); ctx.showToast('Verification upload triggered in Profile! 🎖️', 'ok'); }}>
                  Claim Free (Active Duty / Veterans)
                </button>
              </div>
              
              <button style={{ background: 'none', border: 'none', color: 'var(--muted)', fontSize: 11, marginTop: 14, cursor: 'pointer' }} onClick={() => setShowPremiumModal(false)}>
                Close window
              </button>
            </div>
          </div>
