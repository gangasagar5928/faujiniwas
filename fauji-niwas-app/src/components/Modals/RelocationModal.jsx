...
const LUGGAGE_LIMITS = {
  OR: { weight: 3000, label: 'Other Ranks (OR)', allowance: 18000 },
  JCO: { weight: 6000, label: 'Junior Commissioned Officers', allowance: 34000 },
  Officer: { weight: 12000, label: 'Commissioned Officers', allowance: 78000 }
};

export default function RelocationModal({ onClose }) {
  const ctx = useContext(ModalContext);
  const [activeTab, setActiveTab] = useState('timeline'); // timeline, budget, vault, ecosystem
  ...
  // Standard military allowance formula based on rank allowance baseline + distance factor
  ...
  const simulateRoommateMatch = () => {
    setMatching(true);
    setTimeout(() => {
      // Generate randomized military roommates matching criteria
      const database = [
  ...
  return (
    <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="mc glass-tactical" style={{ maxWidth: 760, width: '95%', height: '88vh' }}>
        ...
        <div className={styles.header} style={{ borderBottom: '1px solid var(--border2)', paddingBottom: 16 }}>
          <div>
            ...
            <p style={{ fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>
              Active Relocation Management Dashboard
            </p>
          </div>
        ...
        <div style={{ display: 'flex', gap: 6, padding: '12px 20px 4px', borderBottom: '1px solid var(--border)', flexWrap: 'wrap' }}>
          {[
            ...
            { id: 'budget', label: '💰 Cost Estimator', title: 'TA/DA Allowance' },
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 20, display: 'flex', flexDirection: 'column' }}>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
                <div>
                  ...
                  <p style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>Rank-specific task scheduler.</p>
                </div>
          ...
          {activeTab === 'budget' && (
            <div>
              <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 4 }}>Luggage & Packing Cost Estimator</h3>
              <p style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 20 }}>
                Calculate official government TA/DA luggage limits versus actual private moving estimates.
              </p>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
                ...
                <div className="glass-tactical" style={{ padding: 16, borderRadius: 12, border: '1px solid var(--border2)' }}>
                  <div style={{ marginBottom: 16 }}>
                    <label style={{ fontSize: 12, color: 'var(--muted)', display: 'block', marginBottom: 6 }}>Posting Distance: <b>{distance} km</b></label>
                    <input
                  ...
                  <div style={{ marginBottom: 16 }}>
                    <label style={{ fontSize: 12, color: 'var(--muted)', display: 'block', marginBottom: 6 }}>Luggage Weight Estimate: <b>{estimatedLuggageWeight} kg</b></label>
                    <input
                    ...
                    <span style={{ fontSize: 10, color: 'var(--muted)', display: 'block', marginTop: 4 }}>
                      Max Government Allowed for {rank}: <b>{maxGovWeight} kg</b>
                    </span>
                  </div>

                  <div style={{ marginBottom: 8 }}>
                    <label style={{ fontSize: 12, color: 'var(--muted)', display: 'block', marginBottom: 6 }}>Movers & Packers Quote: <b>₹{packersCost.toLocaleString()}</b></label>
                    <input
                ...
                {/* Outputs Panel */}
                <div className="bg-mesh" style={{ padding: 18, borderRadius: 12, border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  <div style={{ fontSize: 11, color: 'var(--accent)', fontWeight: 800, textTransform: 'uppercase', marginBottom: 6 }}>TA/DA Allowance Matrix</div>
                  <div style={{ fontSize: 28, fontWeight: 800, color: 'var(--text)' }}>
                    ₹{govReimbursement.toLocaleString()}
                  </div>
                  <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 16 }}>Estimated Government Luggage Reimbursement</div>

                  <div style={{ borderTop: '1px solid var(--border2)', paddingTop: 14 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
                      <span>Packers Expense:</span>
                      <span style={{ fontWeight: 700 }}>₹{packersCost.toLocaleString()}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, fontWeight: 800, color: budgetVariance >= 0 ? 'var(--green)' : 'var(--red)' }}>
                      <span>{budgetVariance >= 0 ? '💼 Budget Savings:' : '⚠️ Out of Pocket:'}</span>
              ...
              <div style={{ background: 'rgba(255,153,51,0.05)', border: '1px solid rgba(255,153,51,0.2)', borderRadius: 10, padding: 12, fontSize: 12, color: 'var(--muted)' }}>
                ℹ️ Allowance estimates are calculated in accordance with the Seventh Central Pay Commission rules for {govLimit.label}. Weight allocations above authorized capacity are capped and paid out-of-pocket.
              </div>
            </div>
          )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <div className="glass-tactical" style={{ padding: 14, borderRadius: 12, border: '1px solid var(--border2)' }}>
                    <h4 style={{ fontSize: 13, fontWeight: 800, color: 'var(--accent)', textTransform: 'uppercase', marginBottom: 8 }}>
                      🧭 SSB Candidates Union Rickshaw Capped Fares
                    </h4>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <div className="glass-tactical" style={{ padding: 14, borderRadius: 12, border: '1px solid var(--border2)' }}>
                    ...
                    <ul style={{ paddingLeft: 14, fontSize: 11, color: 'var(--muted)', display: 'flex', flexDirection: 'column', gap: 6 }}>
                      ...
                      <li>Standard household packaging.</li>
                      <li>Claims assistance for damage.</li>
                    </ul>
                  </div>
                </div>
              </div>
