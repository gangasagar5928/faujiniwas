...
export default function StationModal({ stationId, onClose }) {
  ...
  const stats = useMemo(() => {
    ...
    const isNear = (item, st) => {
      if (!item.lat || !item.lng || !st.lat || !st.lng) return false;
      const d = Math.sqrt(Math.pow(Number(item.lat) - Number(st.lat), 2) + Math.pow(Number(item.lng) - Number(st.lng), 2)) * 111;
      return d < 40; // within ~40km radius
    };
    return {
      count: cityListings.length,
      avgRent,
      schools: ARMY_SCHOOLS.filter(s => isNear(s, station)),
      hospitals: MILITARY_HOSPITALS.filter(h => isNear(h, station)),
      canteens: CANTEENS.filter(c => isNear(c, station))
    };
  }, [station, listings]);
